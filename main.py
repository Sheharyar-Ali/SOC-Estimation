...
def KF_single(T, e1, e2, dt, Cap, R0, R1, R2, v_measured, I):
    # Setting arrays
    ycalc = np.zeros(0)
    Power_used = np.zeros(0)
    Energy_used = np.zeros(1)
    SOC = np.zeros(0)
    SOC_measured = np.zeros(0)
    I1 = np.zeros(0)
    I2 = np.zeros(0)
    # Noise settings
    # The Process noise matrix assumes that the noise is a result of the parameters used in matrices A and B
    ...
    for i in range(0, len(T)):
        x, P, ycalc, SOC, SOC_measured, I1, I2, Energy_used, Power_used = kf_single_input(x, P, ycalc, SOC, I[i], R, Q, I1, I2, SOC_measured, Power_used, Energy_used, v_measured[i], T[0:i])
    return ycalc, SOC, SOC_measured, Energy_used

def KF_single_input_handler(e1, e2, dt, Cap, R0, R1, R2):
    # Setting arrays
    ycalc = np.zeros(0)
    Power_used = np.zeros(0)
    Energy_used = np.zeros(1)
    ...
    while True:
        time, minvoltage, shuntcurrent, shuntcurrentcounter = input("type sth\n").split(',')
        time, minvoltage, shuntcurrent, shuntcurrentcounter = float(time), float(minvoltage), float(shuntcurrent), float(shuntcurrentcounter)
        timeuntilnow.append(time)

        x, P, ycalc, SOC, SOC_measured, I1, I2, Energy_used, Power_used = kf_single_input(x, P, ycalc, SOC, shuntcurrent, R, Q, I1, I2, SOC_measured, Power_used, Energy_used, minvoltage, timeuntilnow)
    return ycalc, SOC, SOC_measured, Energy_used


def kf_single_input(x, P, ycalc, SOC, newI, R, Q, I1, I2, SOC_measured, Power_used, Energy_used, v_measured_new, T_until_now):
    if(len(ycalc) % 500 == 0):
    ...
    Pc = (np.eye(3) - (K * C)) @ Pp
    SOC = np.append(SOC, xc[0][0])
    # SOC[i] = SOC_OCV25deg(y)
    I1 = np.append(I1, xc[1][0])
    I2 = np.append(I2, xc[2][0])
    P = Pc
    x = xc

    ycalc = np.append(ycalc, y)
    SOC_measured = np.append(SOC_measured, SOC_OCV25deg(v_measured_new))
    Power_used = np.append(Power_used, y * newI)
    if len(ycalc) > 1:
        ...
        print(ycalc)
        print(T_until_now)
        Energy_calc = scipy.integrate.simpson(Power_used[0:(len(ycalc))], T_until_now)
        Energy_used = np.append(Energy_used, Energy_calc)

    # if(len(ycalc)%500==0):
    print("SOC:", SOC[-1])
    return x, P, ycalc, SOC, SOC_measured, I1, I2, Energy_used, Power_used

ycalculated, SOC_calculated, SOC_v_min, Energy = KF_single_input_handler(e1=e1, e2=e2, dt=dt, Cap=Cap, R0=R0,
                                                    R1=R1, R2=R2)
